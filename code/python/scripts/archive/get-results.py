...
# Add another one for NEM total at each time step
coincident_droughts = []
# add to this by:
# for time in ratio_da.time:
    # total_coincident = ratio_da.sel(time=time).where(ratio_da < threshold, other=np.nan).notnull().astype(float).sum()
    # coincident_droughts.append((total_coincident, time))

# do this before removing dates from time coord for the above
...
# Take mean of ratio_da
# apply threshold, higher than for regions
# save time

# Actually, can probably can mean and then just save the value directly,
# as this is just a single value so can get a full timeseries
...
if __name__ == '__main__':
    ...
    NOTEBOOK_PATH="/home/548/cd3022/aus-historical-solar-droughts/code/python/notebooks/get-results.ipynb"
    SCRIPT_PATH="/home/548/cd3022/aus-historical-solar-droughts/code/python/scripts/get-results"
